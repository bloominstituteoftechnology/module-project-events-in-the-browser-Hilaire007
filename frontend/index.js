  ...
  document.addEventListener("keydown", (evt) => {
    ...
    if (up) {
      if (targeted.parentElement.previousElementSibling) {
        let index = Array.from(targeted.parentElement.children).indexOf(
          targeted
        );
      ...
      if (targeted.parentElement.nextElementSibling) {
        ...
        targeted.classList.remove("targeted");
        targeted.parentElement.nextElementSibling.children[index].classList.add(
          "targeted"
        );
      }
    } else if (left) {
    ...
    if (!liveBugs.length) {
      ...
      document.querySelector(
        "p.info"
      ).textContent = `All mosquitoes killed in ${elapsedTime / 1000} seconds!`;
      ...
      document
        .querySelector("h2")
        .insertAdjacentElement("beforeend", refreshButton);
      refreshButton.focus();
    }
  });
